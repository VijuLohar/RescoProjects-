...
FS.BookableResourceBooking = {
    ...
    localization: null,
    lastBookingStatusId: null,
    currentBookingStatusId: null,
    defaultFSSettingRecordId: "e49c6117-5065-423f-8ab5-fcacfda85a04",
	...
	// Static variable to hold timestamp frequency - Per Booking status changed/ Per Field Service Status changed.
    timeStampFrequency: null,
    ...
    bookableResourceBookingOnLoad: function () {
        ...
        FS.BookableResourceBooking.getTimestampCreationFrequency();
        ...
        MobileCRM.UI.EntityForm.requestObject(function (entityForm) {
            ...
            if (bookingStatus != null) {
                FS.BookableResourceBooking.lastBookingStatusId = entityProperties[FS.Schema.BookableResourceBooking.properties.bookingStatus].id;
            }
            ...
            if (isNew) {
                ...
                MobileCRM.UI.EntityForm.enableCommand(FS.Common.CustomCommands.customRemoteAssist, false); // #region FieldService
            }
            else
            {
                const msdynWorkOrder = entityProperties && entityProperties[FS.Schema.BookableResourceBooking.properties.msdyn_workOrder];
                if (!msdynWorkOrder) {
    ...
    handleBookingStatusChange: function (entity) {
        ...
        if (bookingStatus == null) {
            ...
            FS.BookableResourceBooking.currentBookingStatusId = entity.properties[FS.Schema.BookableResourceBooking.properties.bookingStatus].id;
    ...
    isFrequencyPerBookingStatusChange: function () {
        if (FS.BookableResourceBooking.timeStampFrequency === FS.Enums.msdyn_fieldservicesetting_msdyn_timetampfrequency.PerBookingStatusChange.toString()) {
            return true;
        }
        return false;
    },

    shouldCreateTimestampForBookingStatusChange: function () {
        return FS.BookableResourceBooking.isFrequencyPerBookingStatusChange() && (FS.BookableResourceBooking.lastBookingStatusId !== FS.BookableResourceBooking.currentBookingStatusId);
    },

    createBookingTimestamp: function (entity, saveHandler) {
        var workOrder = entity && entity.properties && entity.properties[FS.Schema.BookableResourceBooking.properties.msdyn_workOrder];

        if (workOrder && (FS.BookableResourceBooking.lastTimestampStatus !== FS.BookableResourceBooking.currentFSStatus || FS.BookableResourceBooking.shouldCreateTimestampForBookingStatusChange())) {
            ...
            if (FS.BookableResourceBooking.isFrequencyPerBookingStatusChange()) {
                var refer = new MobileCRM.Reference("bookingstatus", FS.BookableResourceBooking.currentBookingStatusId);
                timestampProperties[FS.Schema.BookingTimestamp.properties.msdyn_bookingStatus] = refer;
            }
    ...
    getDefaultBookingCommittedStatus: function () {
     ...
     getTimestampCreationFrequency: function() {
        /// <summary>Fetch the timestampFrequency from msdyn_fieldservicesetting and store it in a static variable</summary>
        var fetchFieldServiceSetting = new MobileCRM.FetchXml.Entity(FS.Schema.FieldServiceSetting.name);
        fetchFieldServiceSetting.addAttribute(FS.Schema.FieldServiceSetting.properties.msdyn_timestampfrequency);
         fetchFieldServiceSetting.filter = new MobileCRM.FetchXml.Filter();
         fetchFieldServiceSetting.filter.where(FS.Schema.FieldServiceSetting.properties.msdyn_fieldServiceSettingId, "eq", FS.BookableResourceBooking.defaultFSSettingRecordId); // default field service setting record id

        var fetchSettings = new MobileCRM.FetchXml.Fetch(fetchFieldServiceSetting);
        fetchSettings.execute("Array", function (results) {
            if (results && results.length > 0) {
                FS.BookableResourceBooking.timeStampFrequency = results[0][0]; //second 0 is the index of the msdyn_defaultBookingCommittedStatus attribute
            }
        }, MobileCRM.bridge.alert, null);
    },
